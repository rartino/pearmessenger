...
export const rtc = (() => {
  ...
  function encodeCode(obj) {
    return CODE_PREFIX + cryptoUtils.b64url.enc(new TextEncoder().encode(JSON.stringify(obj)));
  }

  function decodeCode(code) {
    if (!code.startsWith(CODE_PREFIX)) throw new Error('Invalid code prefix');
    const json = new TextDecoder().decode(cryptoUtils.b64url.dec(code.slice(CODE_PREFIX.length)));
  ...
  async function createInvite(myIdentity, displayName='') {
    ...
    const invite = {
      ...
      me: {
        pub: myIdentity.publicKeyRaw,
        fp: myIdentity.fingerprint,
        name: displayName || ''
      }
    };
  ...
  async function acceptInviteAndCreateAnswer(myIdentity, inviteCode) {
    ...
    // save friend (public key + fingerprint) if not already
    const friend = {
      fingerprint: invite.me.fp,
      name: invite.me.name || 'Friend',
      pub: invite.me.pub,
      lastSeen: Date.now(),
      connected: false
    };
    ...
    const answer = {
      type: 'answer',
      sdp: pc.localDescription.sdp,
      you: { // send my public info to inviter
        pub: myIdentity.publicKeyRaw,
        fp: myIdentity.fingerprint
      }
    };
  ...
  async function setupEncryption(myId, friend, state) {
    if (!state) throw new Error('Missing RTC state');
    // Import keys
  ...
  function onOpen(dc, friend, onStatus) {
    dc.addEventListener('open', () => onStatus(friend.fingerprint, true));
    dc.addEventListener('close', () => onStatus(friend.fingerprint, false));
  }

  return {
    createInvite,
    acceptInviteAndCreateAnswer,
    completeInvite,
    setupEncryption,
    onOpen,
    encodeCode,
